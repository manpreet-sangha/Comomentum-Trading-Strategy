...
def compute_momentum(returns_clean, dates, lookback=48, skip=4):
    ...
    OUTPUTS:
        ...
        momentum_std : TxN np.ndarray - cross-sectionally standardised
    """
    ...
    # Cross-sectionally standardise: at each week t, subtract the
    # cross-sectional mean and divide by the cross-sectional std dev,
    # so that the factor exposures have mean = 0 and std = 1.
    ...
    # Verify standardisation at midpoint
    mid_t = (first_t + last_t) // 2
    sv = momentum_std[mid_t, :]
    sv = sv[np.isfinite(sv)]
    if len(sv) > 0:
        log.info(f"  Standardisation check week {mid_t} ({_d(mid_t)}): "
...
def compute_comomentum(returns_clean, momentum_std, live, ff_factors,
    ...
    INPUTS:
        ...
        momentum_std  : TxN np.ndarray  - standardised momentum scores
...
def compute_adjusted_momentum(momentum_std, comomentum, T, N):
    ...
    INPUTS:
        momentum_std : TxN np.ndarray - standardised momentum exposures
    ...
    # Multiply each stock's standardised momentum by the time-varying scaling
    ...
    # ----- Step D: Re-standardise cross-sectionally -----
    # After scaling, the cross-sectional distribution may have shifted.
    # Re-standardising ensures mean=0, std=1 at each week, which is
    # important for Fama-MacBeth regressions to have consistent units.
    log.info("  Step D: Re-standardising adjusted momentum cross-sectionally...")
    momentum_adj_std = standardiseFactor(momentum_adj)
